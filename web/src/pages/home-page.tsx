...
/** Mobile-first Alfagent landing (< 480px). Desktop uses Alfa HTML clone. */
function MobileHomePage() {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#121212] text-ink">
      <header
        className={cn(
          "fixed inset-x-0 top-0 z-40 transition-[background-color,backdrop-filter,border-color] duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]",
          scrolled || menuOpen
            ? "border-b border-white/10 bg-[#121212]/90 backdrop-blur-xl"
      ...
      >
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
          <Link to="/" className="flex items-center gap-2.5" onClick={() => setMenuOpen(false)}>
            ...
            <span className="text-base font-bold tracking-tight text-white">Alfagent</span>
          </Link>
          ...
          >
            <span
              className={cn(
                "absolute h-0.5 w-5 rounded-full bg-white transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)]",
                menuOpen ? "translate-y-0 rotate-45" : "-translate-y-1.5",
              )}
            />
            <span
              className={cn(
                "absolute h-0.5 w-5 rounded-full bg-white transition-opacity duration-200",
        ...
        <div
          className={cn(
            "border-t border-white/10 bg-[#121212] px-4 pb-6 pt-2",
            menuOpen ? "block" : "hidden",
          )}
        >
          <a
            href="#how"
            className="block rounded-xl px-3 py-3 text-base font-medium text-white/80"
          ...
          <Link
            to="/registration"
            className="mt-3 flex h-12 items-center justify-center gap-2 rounded-xl bg-accent text-sm font-semibold text-white"
            onClick={() => setMenuOpen(false)}
          >
            Открыть бизнес
      ...
      <section className="relative isolate min-h-[100dvh] overflow-hidden bg-[#B36BFF]">
        <img
          src="/line-up.png"
          alt=""
          aria-hidden
          className="pointer-events-none absolute right-0 bottom-0 z-0 h-[42%] w-auto max-w-[88%] object-contain object-right-bottom"
        />
        <div className="relative z-10 mx-auto flex min-h-[100dvh] max-w-6xl flex-col justify-end px-4 pb-10 pt-24">
          <div className="max-w-xl landing-fade">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/75">
              Alfagent
            </p>
            <h1 className="mt-3 text-[2rem] font-bold leading-[1.12] tracking-tight text-white">
              Открой бизнес с сервисами Альфа-Банка
            </h1>
            <p className="mt-4 max-w-md text-[15px] leading-relaxed text-white/90">
              Регистрация ООО онлайн — диалог, документы и протокол в одном месте.
            </p>
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <Link
                to="/registration"
                className="inline-flex h-12 items-center gap-2.5 rounded-xl bg-white px-6 text-sm font-semibold text-ink transition-all duration-200 hover:bg-white/92 active:scale-[0.98]"
              >
                Открыть бизнес
                <span className="flex size-7 items-center justify-center rounded-full bg-ink/8">
          ...
          <ol className="mt-10 grid gap-0">
            {STEPS.map((step, i) => (
              ...
              >
                ...
                <p className="mt-2 max-w-sm text-sm leading-relaxed text-ink-muted">{step.text}</p>
              </li>
            ))}
          </ol>
        </div>
      ...
      <section id="services" className="bg-canvas px-4 py-16">
        <div className="mx-auto max-w-6xl">
...
/**
 * Home: Alfa HTML clone on ≥480px, custom Alfagent landing on phones.
 */
